...
declare namespace Immutable {
  ...
  export interface TypedMap2<T, RecordType> extends TypedMap<T> {
     ...
     * key already exists in this Map, it will be replaced.
     */
    set<K extends keyof T>(key: K, value: T[K]): this & RecordType;
     ...
     * @alias remove
     */
    delete<K extends keyof T>(key: K): this & RecordType;
    remove<K extends keyof T>(key: K): this & RecordType;

    /**
     * Returns a new Map containing no keys or values.
     */
    clear<K extends keyof T>(): this & RecordType;
     ...
     * Equivalent to: `map.set(key, updater(map.get(key, notSetValue)))`.
     */
    update<K extends keyof T>(updater: (value: TypedMap<T>) => TypedMap<T>): this & RecordType;
    update<K extends keyof T>(key: K, updater: (value: T[K]) => T[K]): this & RecordType;
    update<K extends keyof T>(key: K, notSetValue: T[K], updater: (value: T[K]) => T[K]): this & RecordType;
     ...
     *
     */
    merge<K extends keyof T>(...iterables: Iterable<K, T[K]>[]): this & RecordType;
    merge(...iterables: TypedIterable<T>[]): this & RecordType;
    merge<K extends keyof T>(...iterables: Partial<T>[]): this & RecordType;
     ...
     *
     */
    mergeDeep<K extends keyof T>(...iterables: Iterable<K, T[K]>[]): this & RecordType;
    mergeDeep(...iterables: TypedIterable<T>[]): this & RecordType;
    mergeDeep<K extends keyof T>(...iterables: Partial<T>[]): this & RecordType;
     ...
     * `keyPath` do not exist, a new immutable Map will be created at that key.
     */
    setIn(keyPath: Array<any>, value: any): this & RecordType;
    setIn(KeyPath: Iterable<any, any>, value: any): this & RecordType;
     ...
     * @alias removeIn
     */
    deleteIn(keyPath: Array<any>): this & RecordType;
    deleteIn(keyPath: Iterable<any, any>): this & RecordType;
    removeIn(keyPath: Array<any>): this & RecordType;
    removeIn(keyPath: Iterable<any, any>): this & RecordType;
     ...
     *
     */
    withMutations<K extends keyof T>(mutator: (mutable: Map<K, T[K]>) => any): this & RecordType;
    withMutations(mutator: (mutable: TypedMap<T>) => any): this & RecordType;
  ...
  export function OrderedMap<T>(): TypedOrderedMap<T>;
  export function OrderedMap<T>(iter: Iterable.TypedKeyed<T>): TypedOrderedMap<T>;

  export interface TypedOrderedMap<T> extends TypedMap<T> { }
  ...
  export module Record {
    ...
    export type TypedRecord<T> = TypedMap2<T, Readonly<T>> & Readonly<T>;

    export interface TypedClass<T> {
      new (): TypedRecord<T>;
      new (values: Partial<T>): TypedRecord<T>;

      (): TypedRecord<T>;
      (values: Partial<T>): TypedRecord<T>;
  ...
  export module Seq {
    ...
    export function Keyed<T>(): Seq.TypedKeyed<T>;
    export function Keyed<T>(seq: Iterable.TypedKeyed<T>): Seq.TypedKeyed<T>;

    export interface TypedKeyed<T> extends TypedSeq<T>, Iterable.TypedKeyed<T> {
  ...
  export function Seq<T>(): TypedSeq<T>;
  export function Seq<T>(seq: TypedSeq<T>): TypedSeq<T>;
  export function Seq<T>(iterable: TypedIterable<T>): TypedSeq<T>;

  export interface TypedSeq<T> extends TypedIterable<T> {
  ...
  export module Iterable {
    ...
    export function Keyed<T>(iter: Iterable.TypedKeyed<T>): Iterable.TypedKeyed<T>;

    export interface TypedKeyed<T> extends TypedIterable<T> {

      /**
       * Returns Seq.Keyed.
       * @override
       */
      toSeq(): Seq.TypedKeyed<T>;
       ...
       *
       */
      mapKeys<K extends keyof T, M>(
        mapper: (key: K, value: T[K], iter: this) => M,
        context?: any
      ): /*this*/Iterable.Keyed<M, T[K]>;
       ...
       *
       */
      mapEntries<K extends keyof T, KM, VM>(
        mapper: (
          entry: [K, T[K]],
          index: number,
          iter: this
  ...
  export function Iterable<T>(iterable: TypedIterable<T>): TypedIterable<T>;

  export interface TypedIterable<T> {
     ...
     * allow for chained expressions.
     */
    equals(other: TypedIterable<T>): boolean;
     ...
     * The first value in the Iterable.
     */
    first<K extends keyof T>(): T[K];

    /**
     * The last value in the Iterable.
     */
    last<K extends keyof T>(): T[K];
     ...
     * Shallowly converts this iterable to an Array, discarding keys.
     */
    toArray<K extends keyof T>(): Array<T[K]>;
     ...
     * provided for convenience and to allow for chained expressions.
     */
    toOrderedMap(): TypedOrderedMap<T>;
     ...
     * chained expressions.
     */
    toSet<K extends keyof T>(): Set<T[K]>;
     ...
     * for convenience and to allow for chained expressions.
     */
    toOrderedSet<K extends keyof T>(): OrderedSet<T[K]>;
     ...
     * for chained expressions.
     */
    toList<K extends keyof T>(): List<T[K]>;
     ...
     * chained expressions.
     */
    toStack<K extends keyof T>(): Stack<T[K]>;
     ...
     *
     */
    toKeyedSeq(): Seq.TypedKeyed<T>;

    /**
     * Returns an Seq.Indexed of the values of this Iterable, discarding keys.
     */
    toIndexedSeq<K extends keyof T>(): Seq.Indexed<T[K]>;

    /**
     * Returns a Seq.Set of the values of this Iterable, discarding keys.
     */
    toSetSeq<K extends keyof T>(): Seq.Set<T[K]>;
     ...
     * Note: this will return an ES6 iterator which does not support Immutable JS sequence algorithms. Use `keySeq` instead, if this is what you want.
     */
    keys<K extends keyof T>(): Iterator<K>;
     ...
     * Note: this will return an ES6 iterator which does not support Immutable JS sequence algorithms. Use `valueSeq` instead, if this is what you want.
     */
    values<K extends keyof T>(): Iterator<T[K]>;
     ...
     * Note: this will return an ES6 iterator which does not support Immutable JS sequence algorithms. Use `entrySeq` instead, if this is what you want.
     */
    entries<K extends keyof T>(): Iterator<[K, T[K]]>;
     ...
     * discarding values.
     */
    keySeq<K extends keyof T>(): Seq.Indexed<K>;

    /**
     * Returns an Seq.Indexed of the values of this Iterable, discarding keys.
     */
    valueSeq<K extends keyof T>(): Seq.Indexed<T[K]>;

    /**
     * Returns a new Seq.Indexed of [key, value] tuples.
     */
    entrySeq<K extends keyof T>(): Seq.Indexed<[K, T[K]]>;
     ...
     * equivalents will be returned. e.g. `map.sort()` returns OrderedMap.
     */
    sort<K extends keyof T>(comparator?: (valueA: T[K], valueB: T[K]) => number): this;
     ...
     *
     */
    sortBy<K extends keyof T, C>(
      comparatorValueMapper: (value: T[K], key: K, iter: this) => C,
      comparator?: (valueA: C, valueB: C) => number
    ): this;
     ...
     * Note: This is always an eager operation.
     */
    groupBy<K extends keyof T, G>(
      grouper: (value: T[K], key: K, iter: this) => G,
      context?: any
    ): Seq.Keyed<G, this>;
    ...
    forEach<K extends keyof T>(
      sideEffect: (value: T[K], key: K, iter: this) => any,
      context?: any
    ): number;
     ...
     * the resulting iterable, even if they have the same key.
     */
    concat<K extends keyof T>(...valuesOrIterables: Array<Iterable<K, T[K]> | T[K]>): this;
     ...
     * Similar to `iter.map(...).flatten(true)`.
     */
    flatMap<K extends keyof T, MK, MV>(
      mapper: (value: T[K], key: K, iter: this) => Iterable<MK, MV>,
      context?: any
    ): /*this*/Iterable<MK, MV>;
    flatMap<K extends keyof T, MK, MV>(
      mapper: (value: T[K], key: K, iter: this) => /*iterable-like*/any,
      context?: any
    ): /*this*/Iterable<MK, MV>;
     ...
     * @see `Array#reduce`.
     */
    reduce<K extends keyof T, R>(
      reducer: (reduction: R, value: T[K], key: K, iter: this) => R,
      initialReduction?: R,
      context?: any
    ): R;
     ...
     * with `Array#reduceRight`.
     */
    reduceRight<K extends keyof T, R>(
      reducer: (reduction: R, value: T[K], key: K, iter: this) => R,
      initialReduction?: R,
      context?: any
    ): R;
     ...
     * Note: `predicate` will be called for each entry in reverse.
     */
    findLastEntry<K extends keyof T>(
      predicate: (value: T[K], key: K, iter: this) => boolean,
      context?: any,
      notSetValue?: T[K]
    ): [K, T[K]];
     ...
     * Returns the key associated with the search value, or undefined.
     */
    keyOf<K extends keyof T>(searchValue: T[K]): K;

    /**
     * Returns the last key associated with the search value, or undefined.
     */
    lastKeyOf<K extends keyof T>(searchValue: T[K]): K;
     ...
     * that value will be returned.
     */
    max<K extends keyof T>(comparator?: (valueA: T[K], valueB: T[K]) => number): T[K];
     ...
     * that value will be returned.
     */
    min<K extends keyof T>(comparator?: (valueA: T[K], valueB: T[K]) => number): T[K];
     ...
     * True if `iter` includes every value in this Iterable.
     */
    isSubset<K extends keyof T>(iter: Iterable<any, T[K]>): boolean;
    isSubset(iter: TypedIterable<T>): boolean;
    isSubset<K extends keyof T>(iter: Array<T[K]>): boolean;

    /**
     * True if this Iterable includes every value in `iter`.
     */
    isSuperset<K extends keyof T>(iter: Iterable<any, T[K]>): boolean;
    isSuperset(iter: TypedIterable<T>): boolean;
    isSuperset<K extends keyof T>(iter: Array<T[K]>): boolean;
  ...
  export module Collection {
    ...
    export interface TypedKeyed<T> extends TypedCollection<T>, Iterable.TypedKeyed<T> {

      /**
       * Returns Seq.Keyed.
       * @override
       */
      toSeq<K extends keyof T>(): Seq.TypedKeyed<T>;
    }
  ...
  export interface TypedCollection<T> extends TypedIterable<T> {
